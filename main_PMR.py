...
if __name__ == '__main__':
    ...
    if not usehyparams_search:
        clf = lgb.LGBMClassifier(objective='multiclass',n_estimators=50, random_state=42)
    else:
        # clf = XGBClassifier(objective='multi:softmax', eval_metric='mlogloss', num_class=3, use_label_encoder=False,
        #                       random_state=42)
        clf = lgb.LGBMClassifier(objective='multiclass', random_state=42)
        ...
        column_names = ['colsample_bytree', "learning_rate", "max_depth", "min_child_weight", "n_estimators",
                        'subsample', "score"]
    ...
    joblib.dump(scaler, 'mo_scaler.pkl')
    joblib.dump(clf, 'mo_model.pkl')
    ...
    result_df.to_csv('probability_Mo.csv', index=False)